...
const schema = object({
  ...
  startTime: string().optional().default('08:00'), // HH:mm
  duration: number().integer().min(15).max(480).optional().default(60), // minutes
  ...
  return async (req, res, next) => {
    try {
      ...
      const result = await withTransaction(pool, async (client) => {
        ...
        // 4. Fetch component titles from DB (BUG-2 fix: use real names, not generic title)
        const { rows: componentRows } = await client.query<{ id: string; title: string }>(
          `SELECT id, title FROM component_row WHERE id = ANY($1::uuid[])`,
          [body.componentIds]
        );
        const componentTitleMap = new Map(componentRows.map((r) => [r.id, r.title]));

        // 5. Idempotency check (BUG-5 fix): find existing events for same component+date+company
        const { rows: existingEvents } = await client.query<{ component_id: string; event_date: string }>(
          `SELECT
             (metadata->>'componentId')::text AS component_id,
             start_datetime::date::text AS event_date
           FROM company_event
           WHERE company_id = $1
             AND ($2::uuid IS NULL OR class_instance_id = $2)
             AND metadata->>'componentId' = ANY($3::text[])
             AND start_datetime::date >= $4::date
             AND start_datetime::date <= $5::date
             AND status != 'cancelled'`,
          [companyId, body.classInstanceId ?? null, body.componentIds, body.startDate, body.endDate]
        );
        const existingSet = new Set(
          existingEvents.map((e) => `${e.component_id}|${e.event_date.slice(0, 10)}`)
        );

        // 6. Distribute components across dates sequentially (BUG-1 fix)
        //    Instead of all components on every date, assign 1 component per date in order.
        //    Component 1 → date 1, Component 2 → date 2, ... cycle if more dates than components.
        const events: Array<{
          id: string;
          component_id: string;
        ...
        const startTimeStr = body.startTime ?? '08:00';
        const durationMin = body.duration ?? 60;
        let parentEventId: string | null = null;
        let skippedDuplicates = 0;

        // Compute end time from startTime + duration (BUG-4 fix)
        const [startH, startM] = startTimeStr.split(':').map(Number);
        const endTotalMin = startH * 60 + startM + durationMin;
        const endH = Math.floor(endTotalMin / 60) % 24;
        const endMn = endTotalMin % 60;
        const endTimeStr = `${String(endH).padStart(2, '0')}:${String(endMn).padStart(2, '0')}`;

        for (let i = 0; i < dates.length; i++) {
          const componentId = body.componentIds[i % body.componentIds.length];
          const date = dates[i];
          const dateStr = formatDate(date);

          // Idempotency: skip if event already exists for this component+date
          const dupKey = `${componentId}|${dateStr}`;
          if (existingSet.has(dupKey)) {
            skippedDuplicates++;
            continue;
          }

          const componentTitle = componentTitleMap.get(componentId) ?? '';
          const titleText = componentTitle || body.title ||
            `${eventType.charAt(0).toUpperCase() + eventType.slice(1)} - ${dateStr}`;

          const startDt = `${dateStr}T${startTimeStr}:00`;
          const endDt = `${dateStr}T${endTimeStr}:00`;
        ...
        if (events.length === 0) {
          throw Object.assign(
            new Error('All events already exist for the given components and dates (idempotency check). No new events created.'),
            { status: 409 }
          );
        }
