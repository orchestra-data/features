                                  ...
                                  <Select value={selectedComponentId} onValueChange={(v) => handleComponentSelect(v)}>
                                    ...
                                    <SelectContent className="max-h-72">
                                      {unitGroups.find((g) => g.unitId === selectedUnitId)?.components.map((comp) => {
                                        const isScheduled = !!comp.scheduledDate || !!comp.eventId
                                        return (
                                          <SelectItem key={comp.id} value={comp.id} className="py-2">
                                            <div className="flex flex-col gap-0.5">
                                              <div className="flex items-center gap-1.5">
                                                <span className="font-medium">{comp.title}</span>
                                                {isScheduled && (
                                                  <span className="inline-flex items-center rounded-full bg-green-100 px-1.5 py-0.5 text-[9px] font-medium text-green-700">agendado</span>
                                                )}
                                              </div>
                                              <span className="text-[11px] text-muted-foreground">
                                                ...
                                                {comp.scheduledDate ? ` · ${new Date(comp.scheduledDate).toLocaleDateString('pt-BR')}` : ' · sem data'}
                                              </span>
                                            </div>
                                          </SelectItem>
                                        )
