...
function mapToFullCalendarEvent(event: CalendarEventResponse): EventInput {
  ...
  // Show component name in title if linked
  const displayTitle = event.component_name
    ? `${event.title} — ${event.component_name}`
    : event.title;
  ...
  return {
    ...
    extendedProps: {
      ...
      componentDuration: event.component_duration,
