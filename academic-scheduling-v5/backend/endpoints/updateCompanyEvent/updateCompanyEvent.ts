  ...
  return async (req, res, next) => {
    try {
      ...
      await withTransaction(pool, async (client) => {
        ...
        // Epic 5 — R3: Unit overlap validation
        const componentId = existing.component_id ?? null;
        if (componentId) {
          const { rows: compInfo } = await client.query(
            `SELECT c.unit_id, u.series_id, u.title AS unit_title
             FROM component c
          ...
          if (compInfo.length > 0) {
            const { unit_id, series_id } = compInfo[0];
            const eventDate = new Date(startDatetime).toISOString().split('T')[0];

            const { rows: overlaps } = await client.query(
              `SELECT DISTINCT u.title AS conflicting_unit
               FROM company_event ce
               JOIN component comp ON ce.component_id = comp.id
               JOIN unit u ON comp.unit_id = u.id
               WHERE u.series_id = $1
                 AND u.id != $2
                 AND ce.start_datetime::date = $3::date
                 AND ce.deleted_at IS NULL
                 AND ce.status != 'cancelled'
                 AND ce.id != $4
               LIMIT 1`,
              [series_id, unit_id, eventDate, eventId]
            );

            if (overlaps.length > 0) {
              throw Object.assign(
                new Error(
                  `Conflito de unidade: Este horario se sobrepoe com a unidade "${overlaps[0].conflicting_unit}" da mesma serie. Units da mesma serie nao podem ter eventos no mesmo dia.`
                ),
                { status: 409 }
              );
            }
        ...
        // Bidirectional link: sync component's scheduled_date when event date changes
        if (componentId && body.startDatetime) {
          await client.query(
            `UPDATE component SET event_id = $1, scheduled_date = $2::timestamptz, updated_at = NOW()
             WHERE id = $3 AND deleted_at IS NULL`,
            [eventId, body.startDatetime, componentId]
          );
        }
