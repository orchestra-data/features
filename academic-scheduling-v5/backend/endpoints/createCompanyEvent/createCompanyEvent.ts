      ...
      const id = await withTransaction(pool, async (client) => {
        ...
        if (componentId) {
          ...
          if (compInfo.length > 0) {
            ...
            const eventDate = new Date(body.startDatetime).toISOString().split('T')[0];
        ...
        // Bidirectional link: update component's event_id + scheduled_date
        if (body.componentId) {
          await client.query(
            `UPDATE component SET event_id = $1, scheduled_date = $2::timestamptz, updated_at = NOW()
             WHERE id = $3 AND deleted_at IS NULL`,
            [eventId, body.startDatetime, body.componentId]
          );
        }
